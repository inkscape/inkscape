...
匯出時將文字物件轉換成路徑，匯出成 PS、EPS 和 PDF 時適用。
...
將一個 SVG 檔以預設的解析度 90dpi 匯出成 PNG (1 SVG 使用者單位轉變成 1 點陣圖像素)：
...
這份使用說明由 Bryce Harrington E<lt>brycehar@bryceharrington.comE<gt> 彙集整合。
