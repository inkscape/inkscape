...
static int opti_penalty(privpath_t *pp, int i, int j, opti_t *res, double opttolerance, int *convc, double *areac)
{
    ...
    for (k = k1; k != j; k = k1) {
        ...
        if (sign(cprod(pp->curve.vertex[i], pp->curve.vertex[i1], pp->curve.vertex[k1], pp->curve.vertex[k2])) !=
            conv) {
            return 1;
        }
        if (iprod1(pp->curve.vertex[i], pp->curve.vertex[i1], pp->curve.vertex[k1], pp->curve.vertex[k2]) <
...
static int opticurve(privpath_t *pp, double opttolerance)
{
    ...
    for (i = 0; i < m; i++) {
        if (pp->curve.tag[i] == POTRACE_CURVETO) {
            convc[i] =
                sign(dpara(pp->curve.vertex[mod(i - 1, m)], pp->curve.vertex[i], pp->curve.vertex[mod(i + 1, m)]));
        } else {
            convc[i] = 0;
        }
    }
